...
INSERT INTO `curso` VALUES (1,'Curso #1','Descripcion del 1er curso',1200.00,1);
...
--
/*!50003 DROP PROCEDURE IF EXISTS `activar_curso` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `activar_curso`(
	IN var_id INT UNSIGNED
)
BEGIN

	UPDATE curso SET
		estatus = true
	WHERE idCurso = var_id;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `agregar_curso`(
	IN var_nombre varchar (100),
    IN var_descripcion text,
    IN var_precio decimal(15,2) UNSIGNED,
    OUT var_id int UNSIGNED
)
BEGIN

	INSERT INTO curso ( nombre, descripcion, precio ) 
		VALUE ( var_nombre, var_descripcion, var_precio );
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `modificar_curso`(
	IN var_id INT UNSIGNED,
    IN var_nombre VARCHAR(100),
    IN var_descripcion TEXT,
    IN var_precio DECIMAL (15,2) UNSIGNED
...
-- Dump completed on 2023-01-16 22:29:53
